...
const { t } = useTranslation();
  ...
  const gitUsers = async () => {
     setLoading(true);
    try {
      const data = await ApiData().AllAdmin(currentPage, pageSize);
      setUsers(data);
    } catch (err) {
  ...
  const BanUser = async (id) => {
    ...
    const ConvertUser = async (id) => {
    try {
      await ApiData().ConvertUser(id);
      gitUsers();
      toast.success(t("Success"));
    } catch (err) {
      toast.error(err.message);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
         ...
         <Button onClick={() => ConvertUser(record.id)} color="danger" variant="solid">
           {t("ConvertUser")}
          </Button>
      )
    },

    {
      ...
      render: (_, record) => (
                 ...
                 <Button onClick={() => BanUser(record.id)} color="danger" variant="solid">
           {t("bin")}
          </Button>
